...
The expected JSON schema is:
{
  ...
  "version": "<nuvlaedge-engine release>",
...
def arguments():
    ...
    parser.add_argument('--nuvlaedge-installation-trigger-json', dest='nb_trigger_content', default=None, metavar='JSON',
                        help="JSON content, as a string, of the NuvlaEdge installation trigger file")
    parser.add_argument('--nuvlaedge-installation-dir', dest='nb_workdir', default=workdir, metavar='PATH',
                        help="Location on the filesystem where to keep the NuvlaEdge Engine installation files")

    return parser.parse_args()


def prepare_nuvlaedge_engine_installation(version, compose_files, workdir, keep_files=[]):
    """ Prepares the working environment for installing the NuvlaEdge Engine

    :param version: GitHub release of the NuvlaEdge Engine
...
def install_nuvlaedge_engine(cmd, env=os.environ.copy(), timeout=600):
    """ Runs a command
...
if __name__ == "__main__":
    ...
    if previous_conf:
        if "NUVLAEDGE_UUID" in previous_conf:
            previous_uuid = previous_conf['NUVLAEDGE_UUID']
    ...
    if installation_strategy == "OVERWRITE":
        ...
        if nb_vpn_server_id:
            nuvlaedge['vpn-server-id'] = nb_vpn_server_id

        if nb_ssh and "ids" in nb_ssh and isinstance(nb_ssh.get('ids'), list):
            nuvlaedge['ssh-keys'] = nb_ssh.get('ids')

        new_nb_endpoint = nuvla_endpoint + "/nuvlabox"
        nb_id = s.post(new_nb_endpoint, json=nuvlaedge, verify=connection_verify)

        nb_id.raise_for_status()

        nuvlaedge_id = nb_id.json()["resource-id"]
        print("Created NuvlaEdge resource {} in {}".format(nuvlaedge_id, nuvla))
    ...
    try:
        installer_file, compose_files = prepare_nuvlaedge_engine_installation(nb_release,
                                                                             nb_assets,
                                                                             nb_workdir,
                                                                             keep_files=[env_file])
        ...
        install_nuvlaedge_engine(install_command)
    except:
        ...
        if nuvlaedge_id:
            ...
            s.delete(nuvla_endpoint + "/" + nuvlaedge_id, verify=connection_verify)
        raise
