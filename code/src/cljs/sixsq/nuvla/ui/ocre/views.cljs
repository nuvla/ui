...
(defn Pies
      [vouchers]
      (let [group-by-distributor (group-by-distributor (get vouchers :resources []))]
           ...
           [plot/Pie {:height  50
                      ...
                      :data    {:labels   (map :distributor group-by-distributor)
                                :datasets [{:data            (map :quantity group-by-distributor)
                                            :backgroundColor (map :color group-by-distributor)}]}
