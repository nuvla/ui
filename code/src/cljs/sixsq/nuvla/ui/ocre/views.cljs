...
(s/def ::discipline spec-utils/nonblank-string)
...
(s/def ::acl any?)
(s/def ::institute spec-utils/nonblank-string)
(s/def ::researcher-name spec-utils/nonblank-string)
(s/def ::researcher-email spec-utils/nonblank-string)
(s/def ::country #{
...
(s/def ::country-name spec-utils/nonblank-string)
...
(def required-headers #{"code", "amount", "currency", "supplier"})
...
(defn cimi-voucher
  ...
  (->> (select-keys json-line
                    [:amount :currency :code :state :supplier :expiry :platform
                     :distributor :activated :service-info-url :redeemed :wave :batch
                     :discipline :institute :researcher-name :researcher-email :country :country-name])
       (remove #(nil? (second %)))
       (map transform-value)
       (into {})
       (merge {:state "NEW"
