...
(reg-event-fx
  ::create-customer
  (fn [{{:keys [::spec/plan-id] :as db} :db} [_ payment-id]]

    {::cimi-api-fx/add [:customer {:plan_ids          [plan-id]
...
(reg-event-db
  ::set-customer
  (fn [db [_ customer]]
    (assoc db ::spec/customer customer)))
...
;(reg-event-fx
;  ::create-customer
;  (fn [{{:keys [::spec/plan-id] :as db} :db} [_ payment-id email]]
