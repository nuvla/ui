...
test('Changing apps version does not affect other app', async ({ page }, { config }) => {
  ...
  await page.waitForURL(`${baseURL}/ui/apps?apps-store-modules-search=latence`);
  await page
    .getByRole('link', {
    ...
    .click();
  await page.waitForURL(`${baseURL}/ui/apps/latencetech/analyzer/?version=8&apps-tab=overview`);
  await page.getByRole('link', { name: 'Versions' }).click();
  await page.waitForURL(`${baseURL}/ui/apps/latencetech/analyzer/?version=8&apps-tab=versions`);
  await page.locator('a:has-text("v7")').click();
  await page.locator('#nuvla-ui-header-breadcrumb').getByText('latencetech').click();
  await page.waitForURL(`${baseURL}/ui/apps/latencetech?apps-project-tab=overview`);
  await page.getByText('LatenceTech QoSAgent').click();
  await page.getByRole('link', { name: 'Versions' }).click();
  await page.waitForURL(`${baseURL}/ui/apps/latencetech/latencetech?apps-tab=versions`);
