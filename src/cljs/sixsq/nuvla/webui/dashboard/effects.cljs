(ns sixsq.nuvla.webui.dashboard.effects
  (:require-macros
  ...
  (:require
    ...
    [sixsq.nuvla.client.api.runs :as runs]
  ...
  (fn [[client params]]
    (go
      (let [virtual-machines (<! (cimi/search client "virtualMachines" (general-utils/prepare-params params)))]
        (dispatch [:sixsq.nuvla.webui.dashboard.events/set-virtual-machines virtual-machines])))))
  ...
  (fn [[client params]]
    (go
      (let [response (<! (runs/search-runs client params))
        ...
        (dispatch [:sixsq.nuvla.webui.dashboard.events/set-deployments deployments-with-vms])))))

(reg-fx
  ::pop-deleted-deployment
  (fn [[uuid]]
    (go (<! (timeout 30000))
        (dispatch [:sixsq.nuvla.webui.dashboard.events/pop-deleted-deployment uuid]))))
  ...
  (fn [[client uuid]]
    (go
      (let [result (<! (runs/terminate-run client uuid))
        ...
        (if error
          (dispatch [:sixsq.nuvla.webui.dashboard.events/set-error-message-deployment error])
          (dispatch [:sixsq.nuvla.webui.dashboard.events/deleted-deployment uuid]))))))
