(ns sixsq.nuvla.webui.dashboard.views-deployments
  (:require-macros
  ...
  (:require
    ...
    [reagent.core :as r]
    [sixsq.nuvla.webui.client.subs :as client-subs]
    [sixsq.nuvla.webui.dashboard.events :as dashboard-events]
    [sixsq.nuvla.webui.dashboard.subs :as dashboard-subs]
...
(defn terminate-confirm
  ...
  (let [deployment (subscribe [::dashboard-subs/delete-deployment-modal])
        nuvla-url (subscribe [::client-subs/nuvla-url])]
    (fn []
          ...
          :content   (r/as-element
                       [ui/Table {:unstackable true
                        ...
                        [ui/TableBody
                         [ui/TableRow
                          [ui/TableCell "ID:"]
                          [ui/TableCell [:a {:href (str @nuvla-url "/run/" deployment-uuid)} deployment-uuid]]]
                         [ui/TableRow
                          [ui/TableCell "Module URI:"]
                          [ui/TableCell [:a {:href (str @nuvla-url "/" module-uri)} module-uri]]]
...
(defn table-deployment-row
  [deployment]
  (let [nuvla-url (subscribe [::client-subs/nuvla-url])
      ...
      (let [global-prop (if (is-terminated-state? state) {:disabled true} {})
            ...
            row [ui/TableRow (cond
                 ...
                 [ui/TableCell {:collapsing true}
                  [:a {:href (str @nuvla-url "/run/" deployment-uuid)}
                   (-> deployment-uuid
                       (str/split #"-")
                       (first))]]
                 ...
                 [ui/TableCell {:collapsing true :style {:max-width     "150px"
                  ...
                  (let [module-uri-vec (str/split module-uri #"/")
                        module-uri-version (str (nth module-uri-vec
                                                ...
                                                " " (last module-uri-vec))]
                    [:a {:href (str @nuvla-url "/" module-uri)} module-uri-version])]
                 ...
                 [ui/TableCell {:style {:max-width     "100px"
                                        :overflow      "hidden"
                                        :text-overflow "ellipsis"}}
                  [:a {:href (str @nuvla-url "/user/" user)} user]]
